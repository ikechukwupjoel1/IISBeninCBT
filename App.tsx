  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      const foundUser = users.find(u => {
        // Admin Check (Demo Bypass)
        if (u.role === UserRole.ADMIN && regNo === 'admin' && pin === 'admin') {
            return true;
        }

        // Student Check (RegNumber + PIN)
        if (u.role === UserRole.STUDENT) {
            return u.regNumber === regNo && u.pin === pin;
        }

        // Teacher Check (Email + PIN OR Generic Demo)
        if (u.role === UserRole.TEACHER) {
             // Generic Demo
             if (regNo === 'staff' && pin === 'school') return true;
             // Specific Credentials: Check Email and PIN
             // We use 'regNo' state variable to capture the input which could be email
             return u.email === regNo && u.pin === pin;
        }
        
        return false;
      });

      // Fallbacks for Demo purposes if specific user not found in list but using demo creds
      if (foundUser) {
         setCurrentUser(foundUser);
      } else if (regNo === 'admin' && pin === 'admin') {
        // Fallback if the admin user in DB doesn't match exact props but we want to allow entry
        ...
        setCurrentUser(users.find(u => u.role === UserRole.TEACHER) || MOCK_USERS[2]); 
         ...
         // Ensure the mock student can still log in even if they were edited/removed
         const demoStudent = users.find(u => u.regNumber === 'IIS-2024-001') || MOCK_USERS[1];
         if (demoStudent) setCurrentUser(demoStudent);
         else setError('User not found');
      } else {
        setError('Invalid credentials. Please check your ID/Email and PIN.');
      }
      setLoading(false);
    }, 800);
  };
